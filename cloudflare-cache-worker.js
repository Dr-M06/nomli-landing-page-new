 ...
 * 3. Copy this code to Cloudflare Dashboard → Workers → Create Worker
 * 
 * Usage:
...
// GET /cache?key=profile_12345
async function handleGet(request, url, cache, corsHeaders) {
...
async function handlePut(request, cache, corsHeaders) {
  if (!cache) {
    return new Response(JSON.stringify({ error: 'KV namespace not bound' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
...
// DELETE /cache?key=profile_12345
async function handleDelete(request, url, cache, corsHeaders) {
  const key = url.searchParams.get('key');
  if (!key) {
    return new Response(JSON.stringify({ error: 'Key parameter required' }), {
      status: 400,
...
async function handleBatch(request, cache, corsHeaders) {
  if (!cache) {
    return new Response(JSON.stringify({ error: 'KV namespace not bound' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
