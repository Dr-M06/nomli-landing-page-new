      ...
      purchaseUpdatedListener(async (purchase: any) => {
        try {
          ...
          if (vr.success && vr.kind === 'creator_pro') {
            ...
            if (creatorProPurchaseResolve && productId === creatorProPendingGoogleSku) {
...
export const purchaseCreatorProWithGooglePlay = async (
  ...
  try {
    if (!isInitialized) {
      const connected = await initConnection();
      if (!connected) {
        return { ok: false, error: 'Could not connect to Play Billing' };
      }
      isInitialized = true;
    }
    ...
    return await new Promise<{ ok: boolean; error?: string }>((resolve) => {
      ...
      requestSubscription({ sku }).catch((err: any) => {
